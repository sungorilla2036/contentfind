...
export default function VideoPage() {
  ...
  const { platform, videoId } = router.query;
  ...
  useEffect(() => {
    if (platform && videoId) {
      ...
      const loadTranscript = async () => {
        fetch(`${bucketUrl}/${platformNum}/transcripts/${videoId}.json`)
      ...
      const loadClips = async () => {
        fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/videos/${videoId}/clips?platform=${platform}`
        ) // Modified line
          .then((response) => response.json())
          .then((data) => {
            setClips(data);
  ...
  }, [platform, videoId, platformNum, bucketUrl]);
